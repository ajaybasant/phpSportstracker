<?php 
	...
	if (isset($_GET['actn']) && isset($_GET['id'])) {
		...
		$query = "UPDATE enrollment SET status = '$status' WHERE id =".$_GET['id'];
...
<body>
    ...
    <div class="container">
		...
		<?php 
			...
			<?php
			$act_deatils = array();
			$query = "SELECT enrollment.id as aid, enrollment.*,kids.*,activities.* FROM kids JOIN enrollment ON kids.id = enrollment.kidid JOIN activities ON activities.acid = enrollment.actid  WHERE activities.teacher=".$_SESSION['id'];
		...
		<table style="border-width: 2px;">
			...
			<tr>
			    <td><?php echo $no; ?></td>
			    <td><?php echo $act_deatils['firstname']; ?></td>
				...
				<?php
					...
					$new_date = date("d/m/Y", $timestamp);
			 		...
			 		<td>
			 			<?php
			 				if($act_deatils['status']==0){
			 					?>
			 					<a href="viewenrollment.php?actn=aprv&id=<?php echo $act_deatils['aid']; ?>">Approve</a>
			 					<?php
			 				}else{
			 					?>
			 					<a href="viewenrollment.php?actn=rejct&id=<?php echo $act_deatils['aid']; ?>">Reject</a>
			 					<?php
			 				}
			 			?>
			 		</td>
